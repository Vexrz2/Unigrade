  ...
  return (
    ...
    >
      <div className="relative w-full max-w-2xl mx-4 bg-white rounded-lg shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="bg-linear-to-r from-theme4 to-theme3 px-8 py-6 shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold text-white">What-If GPA Calculator</h3>
              <p className="text-theme2 text-sm mt-1">Simulate future courses to see their impact on your GPA</p>
            </div>
            <button 
              type="button" 
              onClick={onClose} 
        ...
        {/* Stats Bar */}
        <div className="grid grid-cols-3 gap-4 p-6 bg-gray-50 border-b shrink-0">
          ...
          <div className="text-center">
            ...
            <p className={`text-2xl font-bold ${difference > 0 ? 'text-green-600' : difference < 0 ? 'text-red-600' : 'text-gray-800'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4">
            {simulatedCourses.map((course, index) => (
              <div key={course.id} className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-theme3 focus:outline-none transition-colors text-sm"
                />
                <input
                  type="number"
                  placeholder="Grade"
                  ...
                  className="w-20 px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-theme3 focus:outline-none transition-colors text-sm text-center"
                />
                <input
                  type="number"
                  placeholder="Credits"
          ...
          <button
            onClick={addCourse}
            className="mt-4 w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-theme3 hover:text-theme3 transition-colors"
          >
            <FiPlus size={18} />
            Add Another Course
          </button>
        ...
        <div className="p-6 bg-gray-50 border-t shrink-0 flex gap-3">
          ...
          <button
            onClick={onClose}
            className="flex-1 bg-linear-to-r from-theme3 to-theme4 hover:shadow-lg text-white font-bold py-3 px-4 rounded-lg transition-shadow duration-200"
          >
            Done
          </button>
        </div>
